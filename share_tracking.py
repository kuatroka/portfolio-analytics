...
def get_userdata(filename):
    ...
    import_a.index = pd.to_datetime(import_a.index, format="%Y-%m-%d")
...
def merge_pricedata(portfolio, index):
    ...
    for i in tickers:
        ...
        inputdata = (yf.Ticker(i).history(start=start_time, auto_adjust=False, interval='3mo')
                    .tz_localize(None))
        inputdata = inputdata.resample('Q').last()
    ...
    # portfolio = portfolio.sort_index().asfreq(freq='3M') 
